...
/**
 * Free transcript fetch via YouTube's public timedtext endpoint.
 * Works for any public video that has captions (auto or manual).
 * No API key, no OAuth.
 *
 * Two-step strategy because YouTube bot-detects datacenter IPs HARD on
 * the watch page (the same way it does for yt-dlp), but the public
 * `/api/timedtext` endpoint stays open — it's the same one that powers
 * the CC widget on every embedded video, so blocking it would break
 * caption display across the whole web.
 *
 *   1. Direct timedtext probe across common languages × {manual, ASR}.
 *      Survives Railway / Vercel / etc. Fast — most attempts 404 in
 *      ~50ms, the right one returns content quickly.
 *   2. Watch-page scrape as a fallback. Works on residential IPs and
 *      occasionally on datacenter ones; the cheap second try costs
 *      nothing if step 1 already won.
 */

// Languages probed in step 1, ordered roughly by global YouTube usage.
// Extending the list trades worst-case latency for coverage — keep it
// short enough that the full sweep stays under ~3s on a cold miss.
const TIMEDTEXT_LANGS = [
  "en",
  "uk",
  "ru",
  "es",
...
/**
 * Hit `/api/timedtext` directly for one (lang, kind) tuple. Returns
 * extracted plain text on success, null on miss / non-200 / empty body.
 * `kind=""` asks for manual subtitles, `kind="asr"` for auto-generated.
 */
export type TimedTextProbeResult = {
  lang: string;
  kind: "" | "asr";
  status: number | "error";
  bodyLength: number;
...
async function probeTimedText(
  videoId: string,
  lang: string,
  kind: "" | "asr"
): Promise<{ text: string | null; debug: TimedTextProbeResult }> {
  ...
  try {
    ...
    const bodySample = body.length > 0 ? body.slice(0, 200) : null;
    // YouTube returns 200 with an empty body when there's no track for
    // (lang,kind) — short-circuit before JSON.parse blows up on "".
    if (!r.ok || !body || body.length < 30) {
      return {
        text: null,
        debug: { lang, kind, status: r.status, bodyLength: body.length, parsedTextLength: 0, bodySample },
      };
    }
    let parsed: TimedTextJson3;
    try {
      parsed = JSON.parse(body) as TimedTextJson3;
    } catch (e) {
      return {
        text: null,
        debug: {
    ...
    const text = (parsed.events ?? [])
      ...
      .replace(/[ \s]+/g, " ")
      .trim();
    // Real transcripts have hundreds of chars at minimum; sub-50-char
    // hits are usually placeholder responses for tracks that exist
    // structurally but have no spoken content (music videos, etc.).
    if (text.length < 50) {
      return {
        text: null,
        debug: { lang, kind, status: r.status, bodyLength: body.length, parsedTextLength: text.length, bodySample },
      };
    }
    return {
      text,
...
/**
 * Same probe sweep as fetchTranscriptFree, but threads each per-probe
 * debug record back to the caller so the /captions route can surface
 * exactly what YouTube said on a miss. Used by the API; chat tools
 * still go through the boolean-shaped fetchTranscriptFree below.
 */
/**
 * Try to pull a transcript via `youtubei.js`, a reverse-engineered Node
 * client that imitates the youtube.com browser session including the
 * Innertube `visitorData` handshake. This bypasses the silent-bot-ban
 * we saw on `/api/timedtext` from Railway datacenter IPs (200 OK +
 * empty body across every language probe).
 *
 * Returns `null` when youtubei can't get a transcript either — at that
 * point we fall back to the legacy timedtext sweep just to be sure,
 * and report both attempts in the debug payload.
 */
/**
 * Innertube exposes different player clients with subtly different
 * server-side defenses. WEB hits the strictest (PO Token required on
 * datacenters → 400). IOS and TV_EMBEDDED use a different signing path
 * and often slip past the bot check from the same IPs. We try them in
 * decreasing order of "tends to work without a PO token" until one
 * gives us a transcript.
 *
 * Order picked from current community findings (yt-dlp issue tracker,
 * youtubei.js discussions late 2025). Subject to change as Google
 * tightens individual clients.
 */
...
/**
 * Pull the signed caption baseUrl out of player info and fetch the
 * XML directly. This is a DIFFERENT endpoint than /v1/get_transcript
 * (which Google is hard-blocking on datacenters with 400 across every
 * player client). The baseUrl carries a session-signed `signature` +
 * `expire` query that Google accepts as proof the request originated
 * from a real player handshake — and Innertube gave us that handshake.
 *
 * Defensive shape navigation because youtubei.js has shipped two or
 * three caption track shapes across major versions.
 */
type CaptionTrackLike = {
  ...
  languageCode?: string;
  kind?: string;
  name?: { text?: string } | string | null;
};

interface PlayerInfo {
  captions?: {
    caption_tracks?: CaptionTrackLike[];
    playerCaptionsTracklistRenderer?: { captionTracks?: CaptionTrackLike[] };
  } | null;
  // Some youtubei versions surface captions under raw/player_response.
  player_response?: {
    captions?: {
      playerCaptionsTracklistRenderer?: { captionTracks?: CaptionTrackLike[] };
    };
...
function extractCaptionTracks(info: unknown): CaptionTrackLike[] {
  const p = info as PlayerInfo;
  const tracks =
    p?.captions?.caption_tracks ??
    p?.captions?.playerCaptionsTracklistRenderer?.captionTracks ??
    p?.player_response?.captions?.playerCaptionsTracklistRenderer?.captionTracks ??
    [];
  return tracks.filter((t) => !!(t.base_url ?? t.baseUrl));
}

async function fetchCaptionTrackXml(
  baseUrl: string
): Promise<{ text: string } | null> {
  try {
    // Some baseUrl values arrive without the `fmt=json3` we want; force
    // it via URLSearchParams. JSON3 is easier to parse than the legacy
    // XML and survives nested HTML entities better.
    ...
    try {
      const parsed = JSON.parse(body) as TimedTextJson3;
      const text = (parsed.events ?? [])
        .flatMap((e) => e.segs ?? [])
        .map((s) => s.utf8 ?? "")
        .join("")
      ...
      // Fall back to the older XML shape: <text>…</text> rows.
      const texts: string[] = [];
      ...
      if (!joined || joined.length < 50) return null;
      return { text: joined };
    }
  } catch {
    return null;
...
async function tryInnertubeTranscript(
  videoId: string
): Promise<{ text: string; language: string; via: string } | { error: string }> {
  try {
    // Dynamic import so the dev bundle doesn't pull in 5MB of YouTube
    // client when other routes load. `retrieve_player: true` so we
    // get the full streaming_data + captions metadata blocks.
    ...
    for (const client of INNERTUBE_CLIENTS) {
      ...
      try {
        ...
        errors.push(
          `${client}/getInfo: ${e instanceof Error ? e.message : String(e)}`
        );
        continue;
      }

      // -------- Trick 1: caption_tracks baseUrl (session-signed) ----
      // Different endpoint than /get_transcript — uses the per-session
      // signature Google attaches when the player loaded a track list.
      // Often slips past the datacenter wall even when /get_transcript
      // hard-400s.
      const tracks = extractCaptionTracks(info);
      if (tracks.length > 0) {
        // Prefer manual subtitles (no `kind`), fall back to ASR.
        const ordered = [
          ...tracks.filter((t) => !t.kind),
        ...
        for (const t of ordered) {
          ...
          if (!url) continue;
          const fetched = await fetchCaptionTrackXml(url);
          if (fetched) {
            const lang = t.language_code ?? t.languageCode ?? "en";
            return {
              text: fetched.text,
              language: lang,
              via: `caption_tracks/${client}`,
            };
          }
        }
      }

      // -------- Trick 2: ask Innertube directly via getTranscript ----
      // Same endpoint that's been 400-ing, kept as a "you never know"
      // last-chance per client. Will silently keep failing in practice
      // until Google rolls back the PO Token requirement.
      try {
        const transcriptData = (await (
          info as { getTranscript?: () => Promise<unknown> }
        ).getTranscript?.()) ?? null;
        if (transcriptData) {
          interface Segment {
            snippet?: { text?: string };
          ...
          type TranscriptShape = {
            transcript?: { content?: { body?: { initial_segments?: Segment[] } } };
            content?: { body?: { initial_segments?: Segment[] } };
            selectedLanguage?: string;
            languages?: string[];
          };
          const raw = transcriptData as unknown as TranscriptShape;
          const segs: Segment[] =
            raw?.transcript?.content?.body?.initial_segments ??
            raw?.content?.body?.initial_segments ??
            [];
          const text = segs
          ...
          if (text.length >= 50) {
            const language =
              raw?.selectedLanguage ?? raw?.languages?.[0] ?? "en";
            return { text, language, via: `get_transcript/${client}` };
          }
        }
      } catch (e) {
        errors.push(
    ...
    return {
      error: `Innertube exhausted across [${INNERTUBE_CLIENTS.join(
        ", "
      )}]: ${errors.join(" | ")}`,
    };
  } catch (e) {
...
export async function fetchTranscriptFreeWithDebug(
  videoId: string,
  opts: { lang?: string } = {}
): Promise<
  | {
      ...
      language: string;
      debug: {
        method: "youtubei" | "timedtext";
        innertubeError?: string;
        probesAttempted: number;
        hitVia: { lang: string; kind: string; bodyLength: number };
        sampleResponses: TimedTextProbeResult[];
      };
    }
  | {
      ok: false;
...
> {
  // ----- Tier 1: youtubei.js (Innertube + signed caption baseUrl) -----
  const inner = await tryInnertubeTranscript(videoId);
  if ("text" in inner) {
    return {
      ok: true,
      text: inner.text,
      ...
      debug: {
        method: "youtubei",
        probesAttempted: 0,
        hitVia: { lang: inner.language, kind: inner.via, bodyLength: inner.text.length },
        sampleResponses: [],
      },
    };
  }
  const innertubeError = inner.error;

  // ----- Tier 2: legacy timedtext sweep -----
  // Kept around because once in a while a video that Innertube refuses
  // (rare languages, geo edge cases) does still cough up a public
  // timedtext track. Cheap secondary attempt.
  const langs = opts.lang
    ? [opts.lang, ...TIMEDTEXT_LANGS.filter((l) => l !== opts.lang)]
    : TIMEDTEXT_LANGS;

  const sampleResponses: TimedTextProbeResult[] = [];
  let probesAttempted = 0;
  for (const lang of langs) {
    for (const kind of ["", "asr"] as const) {
      probesAttempted++;
      const { text, debug } = await probeTimedText(videoId, lang, kind);
      if (sampleResponses.length < 6) sampleResponses.push(debug);
      if (text) {
        return {
          ok: true,
          text,
          ...
          debug: {
            ...
            innertubeError,
            probesAttempted,
            hitVia: { lang, kind: kind || "manual", bodyLength: debug.bodyLength },
            sampleResponses,
          },
        };
      }
...
export async function fetchTranscriptFree(
  videoId: string,
  opts: { lang?: string } = {}
): Promise<{ text: string; language: string } | null> {
  // Step 1: direct timedtext probe. Try the caller's preferred language
  // first (so English-only callers don't pay for 15 misses), then walk
  // the common-language list. Manual subtitles before ASR within each
  // language because they're hand-made and higher quality.
  const langs = opts.lang
    ? [opts.lang, ...TIMEDTEXT_LANGS.filter((l) => l !== opts.lang)]
    : TIMEDTEXT_LANGS;
  for (const lang of langs) {
    for (const kind of ["", "asr"] as const) {
      const { text } = await probeTimedText(videoId, lang, kind);
      if (text) return { text, language: lang };
    }
  }

  // Step 2: legacy watch-page scrape. Datacenter IPs usually get a
  // captions-stripped version of the page, but residential IPs and
  // occasional datacenter ranges still work — cheap to keep as a
  // fallback, and it covers cases where the browser knows about caption
  // tracks the timedtext probe couldn't guess (rare languages, etc.).
  ...
  const tt = await fetch(track.baseUrl, { cache: "no-store" });
  ...
  return { text: joined, language: track.languageCode };
}
